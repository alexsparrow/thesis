\chapter{\acl{SUSY}}
\label{sec:susy}
The \ac{SM}, as described in \chap~\ref{sec:sm} describes all the known
...
\item extending the \ac{SM} gauge group with an additional symmetry or embedding
  the symmetry within a larger group or
...
solves the issues introduced in \sec~\ref{sec:susy_limitations_sm}.
...
\sec~\ref{sec:susy_limitations_sm}. Firstly, it provides a remedy to the
hierarchy problem. The additional superpartners also contribute to the Higgs
mass corrections (see \eqn~\ref{eqn:higgs_mass}). In fact, bosonic and
...
searches. In \chap~\ref{sec:framework}, it will be shown how simplified models
...
\sec~\ref{sec:consequences}. Each fermion in the \ac{SM} is placed into a
...
(see \sec~\ref{sec:theory_ewsb}) leads to the mixing of the gauge eigenstates
...
One such cascade decay is shown in \fig~\ref{fig:susy_1lep_decay}. This would
