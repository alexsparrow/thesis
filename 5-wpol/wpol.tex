...
method described in Section~\ref{sec:wpol_template_reweighting}. The \f0
...
electron channel, it has already been shown that \ac{QCD} multijet events
provide a non-negligible additional contribution. To deal with this, the formula
is extended as follows:
\begin{equation}
N^j = (1 - f_{\textrm{qcd}}) E^j + f_{\textrm{qcd}}Q^j
\end{equation}
where $f_{\textrm{qcd}}$ is the ratio of the QCD multijet background to the
total event yield and $Q^j$ is a shape template derived via the procedure
...
inclusion into the fit - as with the electroweak backgrounds. From the
...
fit. Whilst an independent measurement might perform better - given that the \LP
is not designed to discriminate between electroweak and qcd backgrounds - it was
...
In order to construct an \LP shape template for the \ac{QCD} multijet
background, a data-driven method is needed to extract a sample of fake electrons
which resemble the \ac{QCD} background contamination entering the selected
sample - at least in terms of the \LP shape. The sample of electrons are
obtained by inverting a subset of the selection cuts used in the analysis. The
events passing the inverted set of cuts will be referred to as the
``anti-selected'' sample.

Finding an adequate control sample for this analysis proved challenging. In
order to correctly model the \LP shape, the cuts chosen for inversion must be
relatively uncorrelated with \LP. Since \LP is constructed from both missing
energy and leptonic information in the event, this considerably constrains the
set of suitable variables.
% TODO: EM enriched sample has isolation cut so you cant invert it!
...
full modelling in simulation. This is illustrated in Figure~\ref{wpol_mujecunc}
showing the fractional change of the \LP distribution for positive
muons. Clearly, the effect is quite striking and is strongest in the regions
where the \LP distribution is rising or falling rapidly (i.e. $\LP < 0$ and $\LP
> 1$).
...
As described in Section~\ref{sec:reco_transparency}, the electron momentum is
