...
boson. The functional form of $\sigma$ is taken from
...
\end{eqnarray*}
By weighting a given simulated event by each of the factors $Q_i$, any given
distribution can be made to represent the 100\% left-handed, right-hadned and
longitudinal polarisation cases without having to alter the configuration of the
Monte-Carlo generator. Using this method, templates can be constructed for the
\LP distribution corresponding to a pure sample of polarised bosons and
accounting for all detector and acceptance effects. Such \LP templates derived
from Monte Carlo may then be fit to the corresponding data distributions in
order to extract the helicity fractions.
