...
\end{equation}
Finally, using the fact that for \PWpm, $M=\pm1$ and replacing for the elements
of the D-matrices in terms of $\cos\thetastar$
\begin{equation}
...
\end{equation}
Comparing now to Eqn~\ref{eqn:wpol_xs_Ai}, we identify $A_0 \sim f_0$ and $A_4
\sim \pm \fLmfR$.

\section{Measuring the Helicity Fractions}
\subsection{Probing $\cos\thetastar$ via \LP}
...
from the missing momentum. This prevents determination of the \PW momentum and
thus too the direct measurement of $\cos\thetastar$. Instead, a variable is
chosen which is known to be highly correlated with $\cos\thetastar$ at suitably
high $\PtW$ and yet directly calculable from transverse detector quantities
which can be easily measured. This variable is the Lepton Projection variable or
\LP and is defined as follows
\begin{equation}
...
% TODO It would be nice to give some mathematical motivation for this expression
