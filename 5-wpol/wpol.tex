...
then fixed to be perpendicular to these two. The azimuthal angle, \thetastar is
measured in the $y-z$ plane between the positive z-axis and the
lepton. Likewise, the polar angle, \phistar is measured in $x-z$ between the
...
polarisation state. Most importantly, templates can be constructed for the \LP
distribution corresponding to a pure sample of polarised bosons and accounting
for all detector and acceptance effects. Such \LP templates derived from Monte
Carlo may then be fit to the corresponding data distributions in order to
extract the helicity fractions.
