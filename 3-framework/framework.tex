...
since much of the beam-beam remnant escapes along the beamline, outside
...
\begin{figure}
  ...
  \caption[Illustration of a $\Pproton\APproton$ collision]{Illustration of a
    ...
    occurred. The underlying event is also shown, along with initial and final
    state radiation~\cite{underlying_event}. }
...
In the case where the \PW boson carries a significant transverse momentum, \PtW,
the situation is more complex. For the sake of this discussion we will consider
...
lepton will have a larger rapidity than the \PW boson and thus a
...
\caption[Illustration of the helicity frame]{Illustration of the helicity
  frame. The lines $P_1$ and $P_2$ represent the
  ...
  \thetastar, are also shown~\cite{berger_left_handed_w}.}
...
\begin{equation*}
\frac{d\sigma^{h_1 h_2}}{d\left(\PtW\right)^2 d\YW d\Omega^*} = \sum_{ab} \int dx_1 dx_2
f_a^{h_1}\left(x_1, \mu_F^2\right)f_b^{h_2}\left(x_2, \mu_F^2\right)
\frac{s d\tilde{\sigma}_{ab}}{dt du d\Omega^*} \left(x_1 P_1, x_2 P_2,
\alpha_s (\mu_R^2)\right),
\end{equation*}
where $d\Omega^* = \sin\thetastar d\thetastar d\phistar$, $\thetastar$ and
$\phistar$ are the polar and azimuthal decay angles of the leptons in the
helicity frame and \YW is the rapidity of the \PW boson.  $h_1$ and $h_2$ are
the interacting hadrons and the sum runs over $a,b = \Pquark, \APquark,
\Pgluon$. The \acp{PDF}, $f_a^{h}\left(x, \mu^2\right)$, give the probability of
finding a parton $a$ with momentum fraction $x$ in hadron $h$ when probed at a
scale $\mu^2$. The $d\tilde{\sigma}_{ab}$ are the parton-level cross-sections
for the chosen process(es). The hadron-level Mandelstam variables are written in
uppercase,
\begin{equation*}
...
interacting partons, $p_1$ and $p_2$ and the \PW boson, $Q$. The cross-section
can be decomposed in terms of a standard set of angular coefficients, $A_i$, to
give~\cite{berger_left_handed_w,mirkes_w_1992,strologas_thesis}
\begin{align}
\label{eqn:wpol_diff_xs}
\frac{1}{\sigma} \frac{d\sigma}{d\left(\cos\thetastar\right)d\phistar} = \frac{3}{16\pi}
&\left [ \left(1+\cos^2\thetastar\right) \right. \nonumber\\
 ...
 &+ A_4\cos\thetastar + A_5\sin\thetastar\sin\phistar  \nonumber\\
 &+ \left. A_6\sin 2\thetastar\sin \phistar + A_7\sin^2\thetastar\sin 2\phistar \right ],
\end{align}
...
where $\sigma$ may be any differential cross-section that does not depend on the
individual lepton kinematics~\cite{berger_left_handed_w}. The $A_i$ are ratios
of the separate helicity cross-sections of the boson to its total unpolarised
cross-section. They are dependent on the \PW boson charge, transverse momentum,
\PtW, and rapidity, \YW. \eqn~\ref{eqn:wpol_diff_xs} can be integrated over
$\phistar$ to give
\begin{equation}
...
neutrino. Therefore the angular momentum state of the decay leptons is
\begin{eqnarray*}
...
Whilst the dependence of the polarisation on \PtW makes for a very interesting
