...
In such supersymmetric models, the gluino may decay only via an off-shell squark
\cite{alwall_simplified}. This may be either directly to the LSP or indirectly
via intermediate states. Direct decays correspond to \ac{SUSY} scenarios where
either \cite{alves_simplified_2011}:
\begin{itemize}
...
parameterised by the mass of the mother particle, $m_{\PSgluino}$, the mass of
the daughter particle, $m_{\textrm{LSP}}$ and the mass of the intermediate
particle, $m_{\PScharginopm}$.
...
assumed to be stop particles - the superpartner of the top quark. This reflects
the scenario in which the stop is the lightest of the squarks. This is
theoretically attractive (see \cite{sparticles}, p202 and
\cite{light_stop}). The \Ttwott topology is illustrated in
Figure~\ref{fig:sms_topologies_t2tt}. Since this does not contain an
intermediate mass state, it has only two paramters: the mass of the mother,
$m_{\PSgluino}$ and the mass of the daugther, $m_{\textrm{LSP}}$.
