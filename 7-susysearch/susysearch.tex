...
chapter, to discrimate \ac{SUSY} events from \ac{SM} backgrounds. In the
following sections, a \ac{SUSY} search applying these ideas will be
described~\cite{susy_ra4_pas}.

\section{Distinguishing \ac{SUSY} and \ac{SM} Events}
\label{sec:susy_sm}
...
events, whilst suppressing \ac{EWK} backgrounds in this sample, a $\MET <
\unit{20}{\GeV}$ cut is also applied. Even with these cuts, significant \ac{EWK}
contamination remains. The significance of the vertex impact paramter is used as
an additional handle, by requiring $\sigma(\D0) > 3$. By selecting muons
...
shape of the QCD template was affected by a cut on \PtW. Thus it should be
...
To account for the \ac{QCD} contamination in the control region, the background
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.5\textwidth]{fig/SelectedVsAntiSelected}
...
The effect of limited statistics in the control region is naturally more
...
region. For electrons, this is taken as the uncertainty on the \ac{EWK}
component of the background as determined from the fit (see
\sec~\ref{sec:susy_electron_bgpredict}).

\subsection{Limited \acl{MC} Statistics}
...
fitting procedure. The \ac{EWK} templates used in the fit also suffer from
...
times according to the statistical uncertainty per bin. Each re-diced template is
then used to repeat the fit procedure. The variance of this ensemble of fits is
then taken to be the statistical uncertainty. For the systematic uncertainties
quoted in Table~\ref{tbl:susy_syst_muons}, this is propagated into \RCS. For the
limit, as will be seen, it is taken as an uncertainty on \NControl.
...
case, this is a conservative estimate.
...
a cross-section measurmement at \ac{CMS}~\cite{cms_ttbar_paper2} (improved with
...
simultaneously for both lepton channels, in order to maximise available
statistics.
...
this, the simulation was reweighted to reflect an upward and downard shift of
...
\begin{figure}[h!]
\centering
\subfloat[]{\label{fig:susy_wpol_syst_plus_150}\includegraphics[width=0.49\textwidth]{fig/e_plus_fLmfR_150.pdf}}
\subfloat[]{\label{fig:susy_wpol_syst_minus_150}\includegraphics[width=0.49\textwidth]{fig/e_minus_fLmfR_150.pdf}}\\
\subfloat[]{\label{fig:susy_wpol_syst_plus_450}\includegraphics[width=0.49\textwidth]{fig/e_plus_fLmfR_450.pdf}}
\subfloat[]{\label{fig:susy_wpol_syst_minus_450}\includegraphics[width=0.49\textwidth]{fig/e_minus_fLmfR_450.pdf}}\\
...
sets. The resulting change in \RCS was found to be 1\% for $\STlep <
