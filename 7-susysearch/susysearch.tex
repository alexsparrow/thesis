...
paramter is used as an additional handle by requiring $\sigma(D_0) > 3$. The
ratio $N(\CombIso < 0.1/N(0.2 < \CombIso < 0.5)$ in this sample is then used to
derive an upper limit on the \ac{QCD} contribution. This limit is conservative
given that electroweak background are present in the ratio. It is seen in
Table~TODO that the \ac{QCD} background is negligible in all \STlep bins and is
...
\begin{figure}
\centering
\includegraphics[width=0.7\textwidth]{fig/SelectedVsAntiSelected}
...
\subsection{\acl{JES}Uncertainty}
...
resulting shift in \RCS is taken as the uncertainty.

\subsection{\Wjets/\ttbar Cross Section}
...
\subsection{\acl{PDF} Uncertainties}
...
central value of both \cteqsixtysix~\cite{cteq66} and \mstwnlo~\cite{mstw1,
  mstw2, mstw3} \ac{PDF} sets and the resulting change in \RCS was found to be
1\% for $\STlep < \unit{450}{\GeV}$ and 5\% for $\STlep >
\unit{450}{\GeV}$. This systematic uncertainty is therefore seen to be
negligible.
