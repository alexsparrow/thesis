\ctable[
cap=Event yields in the muon control sample,
caption=Event yields in the \LPsignal and \LPcontrol regions for the $<3$~jet
control sample in the muon channel. Expected \ac{MC} yields are shown for each
\ac{SM} background and as a total. The ``SM Estimate'' row gives the \ac{SM}
background as predicted from the control region using the \ac{MC} translation
factor \RCS. The first uncertainty is from the statistical uncertainty on the
yield in the control region. The second is from the statistical uncertainty on
\RCS due to the limited \ac{MC} sample. The expected yield for a benchmark
\ac{SUSY} point\, \ac{LM6}\, is also shown.,
pos=h!,
