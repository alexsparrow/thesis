...
\begin{figure}[h!]
  ...
  \caption[Diagram illustrating electron conversion rejection
  variables}]{Diagram illustrating the electron conversion rejection variables,
  \Dist and \DeltaCotTheta. \Distnm is the distance between points B1 and B2 in
  the $x-y$-plane. Here, the two tracks from the photon conversion are
  parallel. \Dist is negative when the two tracks overlap and positive
  otherwise.}
...
\begin{figure}[h!]
\centering
\subfloat[]{\label{fig:reco_pf_diag1}\includegraphics[width=0.7\textwidth]{fig/pf_diag1}}\\
\subfloat[]{\label{fig:reco_pf_diag2}\includegraphics[width=0.45\textwidth]{fig/pf_diag2}}\quad
\subfloat[]{\label{fig:reco_pf_diag3}\includegraphics[width=0.45\textwidth]{fig/pf_diag3}}
\caption[\ac{PF} event display showing a hadronic jet]{An event display showing
  ...
  photons result in clear, well-speparated \ac{ECAL} cluters -- round points
  labelled E1 to E4~\ref{fig:reco_pf_diag2}. The \Ppiplus and \Ppiminus are
...
\caption[\ac{PF} \MET energy and angular resolution in \ttbar events]{\subref{fig:reco_pf_met_metres} $\sigma(\MET)/\MET^{\textrm{true}}$
