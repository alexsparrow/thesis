...
\begin{equation}
N^i_{\textrm{exp}} = N^i_{\textrm{signal}}(\mu, \nu_j) +
N^i_{\textrm{bkg}}(\nu_k) \\
\end{equation}
where $N_{\textrm{signal}}$ is the expected signal yield for a chosen
signal model and $N_{\textrm{bkg}}$ is the data-driven background
...
\begin{equation}
\mathcal{L} = \prod_i \mathcal{P}(N^i_{\textrm{exp}} ; N^i_{\textrm{obs}})
\prod_{k \in \Theta_i} X(\nu_{ik})
\prod_{j \in \Theta}  Y(\nu_j)
\end{equation}
where
\begin{itemize}
...
$x$
\item $X(\nu_i)$ denotes an unspecified statistical distribution used to
  model the uncorrelated statistical uncertainty on the background prediction in
  each bin. The $\nu_i$ are nuisance parameters.
\item $Y(\nu_j)$ denotes an unspecified statistical distribution used to model
  the correlated systematic uncertainties. The $\nu_j$ are nuisance parameters.
\end{itemize}
