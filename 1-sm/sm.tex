...
\begin{equation*}
  ...
  - \frac{1}{2}m^2\left|\phi\right|^2 - \frac{\lambda}{4!}\left|\phi\right|^4.
\end{equation*}
...
matrices. The parameter $\alpha^a(x)$ is the analogue of $\theta(x)$ in
...
where
\begin{equation*}
\phi' = \frac{1}{\sqrt{2}}\left(\phi'_1 + i\phi'_2\right).
\end{equation*}
...
\end{align}
Two things should be noted about the resulting Lagrangian. Firstly, that the
second term implies the existence of a scalar field with mass
$\sqrt{-2\mu^2}$. Secondly that the vector boson $A_{\mu}$ has acquired a mass,
$gv$ -- due to the second-to-last term in \eqn~\ref{eqn:higgs_lag}. The last
term in \eqn~\ref{eqn:higgs_lag} is problematic, but can be removed by moving to
the \emph{unitary gauge}~\cite{peskin_schroeder}.
