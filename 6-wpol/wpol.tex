\chapter{Measuring the Polarisation of the \texorpdfstring{$\PW$}{W} Boson}
\label{sec:wpol}
...
\ac{MADGRAPH}~\cite{madgraph} generator interfaced to
\ac{PYTHIA}~\cite{pythia}. The generated sample comprised approximately 15
million events with the \acp{PDF} taken from the ac{CTEQL1} set within the
\ac{LHAPDF} software package.
...
\begin{figure}
\centering
\subfloat[Two-dimensional distribution of \YW vs \PtW at generator level]{
  \label{fig:wpol_genreco_wpt_eta}\includegraphics[width=0.47\textwidth]{fig/WPTvsY_mcreco50toinf}}\quad
\subfloat[The \PtW distribution at generator level after a reconstruction level
cut of \unit{50}{\GeV}]{
...
matrix element generator interfaced to
\ac{PYTHIA}. In the case of the \ac{QCD} background, a number of
samples were used. In the case of the Muon channel, this was a sample generated
using \ac{PYTHIA} and binned in terms of the transverse momentum of the hard
interaction, \pthat. This ensures adequate statistics even in the regions of
high \pthat that will tend to pass the analysis selection.
...
particle flow algorithm, as discussed in Section~\ref{sec:reco_pf}. The leading
source of uncertainty is due to the callibration of the jet energy scale.
...
measurement~\cite{w_charge_asymmetry}. The detector is divided into 6 $\eta$
bins. The \Zee mass distribution in data is then divided into $6 + ^6C_2$
...
resolution term $\sqrt{\sigma_i^2 + \sigma_j^2}$. The end result of this is a
set of 6 scale terms and 6 resolution terms. These are plotted in
...
\begin{figure}
  \centering
  \subfloat[Scale]{\label{fig:wpol_ecal_transp_scale}\includegraphics[width=0.45\textwidth]{fig/ecal_transp_scale}}\quad
  \subfloat[Resolution]{\label{fig:wpol_ecal_transp_sigma}\includegraphics[width=0.45\textwidth]{fig/ecal_transp_sigma}}\quad
\caption[]{\ac{ECAL} transparency correction factors as a function of
  pseudorapidity for \subref{fig:wpol_ecal_transp_scale} scale and
  \subref{fig:wpol_ecal_transp_sigma} resolution~\cite{w_charge_asymmetry_an}}
