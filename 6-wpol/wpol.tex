...
\begin{equation*}
|\Plhpar| = \frac{\MW}{2}\cos\thetastar \qquad
...
\begin{eqnarray*}
\LP^{3D} &=& \frac{|\Pll|}{\mPW} = \frac{1}{\mPW}\sqrt{|\Pllpar|^2 + |\Pllperp|^2}\\
...
\YWm. These can be written as
\begin{equation}
\label{eqn:wpol_reweighting_factor}
Q_i^\pm\left(\cos\thetastar, \PtW, \YWm \right) =
\frac{\sigma^\pm_i\left(\cos\thetastar\right)}{\displaystyle\sum_{j=-1}^{j=+1}
  \ffj^\pm\left(\PtW, \YWm\right)\sigma^\pm_j\left(\cos\thetastar\right)},
\end{equation}
where the index, $i$ is one of the three helicity states of the \PW boson. The
$\ffj^\pm$ are constants derived from an analytical fit to the $\cos\thetastar$
distribution in bins of \PtW, \YWm and charge. These are effectively the helicity
fractions ``baked in'' to the \ac{MC}. The factor, $Q_i^\pm$, will then re-weight
the $\cos\thetastar$ distribution in \ac{MC} such that it corresponds to the
pure helicity state, $i$. The functional forms of the $\sigma_i^\pm$ are taken from
...
\ffj^\pm\left(\PtW, \YWm\right)\sigma^\pm_j\left(\cos\thetastar\right)}{
\displaystyle\int_{-1}^{1} \sum_{j=-1}^{j=+1} \ffj^\pm\left(\PtW, \YWm\right)\sigma^\pm_j\left(\cos\thetastar\right)
}
\end{equation*}
...
integration elements, $d(\cos\thetastar)$, have been suppressed.
...
\begin{equation*}
R_i^\pm\left(\PtW, \YWm \right) =
\frac{{f'}_i^\pm \left(\PtW, \YWm \right)}{{f'}_i^{\pm,\,\textrm{all}}},
\end{equation*}
where ${f'}_i^\pm\left(\PtW, \YWm \right)$ is the fraction of \PWpm bosons in the
appropriate \PtW and \YWm bin with helicity $i$. ${f'}_i^{\pm,\,\textrm{all}}$ is the
...
To summarise, the re-weighting procedure depends on two factors: $Q_i^\pm$ and
$R_i^\pm$. The first ``alters'' the shape of the $\cos\thetastar$ distribution to
...
The requirements placed on the muon are as follows~\cite{cms_mu_pas,cms_wz_pas,cms_w_paper}.
\begin{itemize}
...
\begin{figure}[h!]
  \centering
  \subfloat[]{\label{fig:wpol_mht_res1}\includegraphics[width=0.49\textwidth]{fig/mht_res1_fixed.png}}
  \subfloat[]{\label{fig:wpol_mht_res2}\includegraphics[width=0.49\textwidth]{fig/mht_res2_fixed.png}}
...
these both contain actual decays of a heavy particle. For the majority of
\ac{QCD} events passing the analysis selection, a balanced jet system has been
badly reconstructed such that one of the jets has been misidentified as a
lepton. In general, the \MET in these events will be small and thus rejected by
the \MT cut. In the rare case that the \MET component is large due to
significant jet mismeasurement, there are two possibilities.
\begin{itemize}
\item The fake \MET is collinear with the jet that has been misidentified as a
  lepton. Since $\Delta\phi \sim 0$, the \MT cut should strongly suppress these
  events.
\item The angle between the fake \MET and lepton is large, leading to significant
  \MT. Note however, that these events will in general be suppressed by the cut
  on \PtW.
\end{itemize}
\ac{QCD} events surviving the combined \PtW and \MT cuts tend towards larger
values of \Ptl.
...
\subsection[\acs{QCD} and \texorpdfstring{\gammajets}{Photon+Jets} Backgrounds]{\acs{QCD} and \boldmath{\gammajets} Backgrounds}
...
\begin{figure}[h!]
\centering
\subfloat[80\% efficiency selection]{\label{fig:wpol_wp80}\includegraphics[width=0.4\textwidth]{fig/ICVarPlusMT50_WP80_hacked_2}}\quad
\subfloat[70\% efficiency selection]{\label{fig:wpol_wp70}\includegraphics[width=0.4\textwidth]{fig/ICVarPlusMT50_WP70_hacked_2}}
...
\begin{figure}[h!]
\centering
\subfloat[$\PtW > \unit{50}{\GeV}$]{\label{fig:wpol_antiselected1}\includegraphics[width=0.4\textwidth]{fig/antiselected1}}\quad
\subfloat[$\MT > \unit{50}{\GeV}$]{\label{fig:wpol_antiselected2}\includegraphics[width=0.4\textwidth]{fig/antiselected2}}
...
\begin{figure}
\centering
\subfloat[$\Ptl\left(\APelectron\right)$]{\label{fig:wpol_datamc_ele_leppt_plus}\includegraphics[width=0.32\textwidth]{fig/ele_LeptonPtPlus}}
\subfloat[$\LP\left(\APelectron\right)$]{\label{fig:wpol_datamc_ele_lp_plus}\includegraphics[width=0.32\textwidth]{fig/ele_ICVarPlus}}
\subfloat[$\PtW\left(\APelectron\right)$]{\label{fig:wpol_datamc_ele_wpt_plus}\includegraphics[width=0.32\textwidth]{fig/ele_WPtPlus}}\\
\subfloat[$\Ptl\left(\Pelectron\right)$]{\label{fig:wpol_datamc_ele_leppt_minus}\includegraphics[width=0.32\textwidth]{fig/ele_LeptonPtMinus}}
\subfloat[$\LP\left(\Pelectron\right)$]{\label{fig:wpol_datamc_ele_lp_minus}\includegraphics[width=0.32\textwidth]{fig/ele_ICVarMinus}}
\subfloat[$\PtW\left(\Pelectron\right)$]{\label{fig:wpol_datamc_ele_wpt_minus}\includegraphics[width=0.32\textwidth]{fig/ele_WPtMinus}}\\
\subfloat[$\MET\left(\Pe\right)$]{\label{fig:wpol_datamc_ele_pfmet}\includegraphics[width=0.32\textwidth]{fig/ele_pfMET}}
\subfloat[$\MT\left(\Pe\right)$]{\label{fig:wpol_datamc_ele_mt}\includegraphics[width=0.32\textwidth]{fig/ele_MT}}
...
\begin{figure}
\centering
\subfloat[$\Ptl\left(\APmuon\right)$]{\label{fig:wpol_datamc_mu_leppt_plus}\includegraphics[width=0.32\textwidth]{fig/datamc_ptmuonplus}}
\subfloat[$\LP\left(\APmuon\right)$]{\label{fig:wpol_datamc_mu_lp_plus}\includegraphics[width=0.32\textwidth]{fig/datamc_lpvarplus}}
\subfloat[$\PtW\left(\APmuon\right)$]{\label{fig:wpol_datamc_mu_mt_plus}\includegraphics[width=0.32\textwidth]{fig/datamc_recoptwplus}}\\
\subfloat[$\Ptl\left(\Pmuon\right)$]{\label{fig:wpol_datamc_mu_leppt_minus}\includegraphics[width=0.32\textwidth]{fig/datamc_ptmuonminus}}
\subfloat[$\LP\left(\Pmuon\right)$]{\label{fig:wpol_datamc_mu_lp_minus}\includegraphics[width=0.32\textwidth]{fig/datamc_lpvarminus}}
\subfloat[$\PtW\left(\Pmuon\right)$]{\label{fig:wpol_datamc_mu_mt_minus}\includegraphics[width=0.32\textwidth]{fig/datamc_recoptwminus}}
...
uncertainties. In each case, the correlation between the polarisation parameters
is shown along with the $\chisq/\textrm{ndof}$ measure of the
goodness-of-fit. Table~\ref{tbl:fqcd_fit_results} shows the results for the
parameter \fQCD in the electron-only and combined fits.
...
\subfloat[\Pem]{\label{fig:wpol_contour_ele_minus}\includegraphics[width=0.45\textwidth]{fig/electron_contour_minus}}
...
\begin{figure}
\centering
\subfloat[\Pep/\Pgmp]{\label{fig:wpol_contour_comb_plus}\includegraphics[width=0.45\textwidth]{fig/combined_contour_plus}}\quad
\subfloat[\Pem/\Pgmm]{\label{fig:wpol_contour_comb_minus}\includegraphics[width=0.45\textwidth]{fig/combined_contour_minus}}
