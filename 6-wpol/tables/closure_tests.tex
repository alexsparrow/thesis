\ctable[
cap=Fit results for the helicity fractions $f_{L}^{\pm}$ and $f_{R}^{\pm}$
with respect to several closure tests of the reweighting method described in
Section~\ref{sec:wpol_reweighting},
caption=Fit results for the helicity fractions $f_{L}^{\pm}$ and $f_{R}^{\pm}$
with respect to several closure tests of the reweighting method described in
Section~\ref{sec:wpol_reweighting}. The results of an analytical fit to the
$\cos\thetastar$ distribution\, having applied reconstruction level cuts on the
\PW boson\, is shown in the first column. The second column shows results from fits to the
