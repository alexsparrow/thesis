...
distribution~\cite{wilks_theorem,statistical_methods,param_estimation}. The number of degrees
...
\caption[The negative log \acs{PL} ratio as a function of $\mu$]{The negative log
  \ac{PL} ratio as a function of $\mu$ \subref{fig:inter_pl_80_400_syst} with
  different systematic effects included in the likelihood and
...
\fig~\ref{fig:inter_pl} shows the negative log \ac{PL} ratio, $-\ln\lambda$, as
a function of the signal strength parameter, $\mu$. The signal strength is
defined as the ratio of the considered cross-section to that predicted by the
\ac{CMSSM}. As expected, the inclusion of systematic effects pushes the
exclusion to larger values of $\mu$. A similar, but more pronounced, effect can
be seen when removing the electron channel. This shows, qualitatively at least,
that the model is behaving as expected. For this particular model point, the
signal contamination appears to have the largest effect, followed by the
background uncertainties and then the signal uncertainties. Note that certain
uncertainties are included in all cases: the effect of the statistical
uncertainty in the control region, and the luminosity uncertainty. The former is
actually the dominant uncertainty on the background prediction. This perhaps
explains why the addition of the other systematic uncertainties appears to have
...
\prospino package~\cite{prospino} and \softsusy is used to solve the
...
For the simplified models shown here, the mother and daughter particle's masses
...
models with \Mchargino close to \Mgluino appear to give better exclusion. In
