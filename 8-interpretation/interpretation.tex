...
\ac{NP} models: \ac{mSUGRA} and the \T2tt simplified model described in
Section~TODO.
...
\subsection{\T2tt}[h!]
Similary the efficiency per \STlep bin with respect to the \T2tt parameter
space is shown in Figures~\ref{fig:inter_t2tt_mu} and \ref{fig:inter_t2tt_el}
respectively.

For the \T2tt result it is desirable to have not only an exclusion plot, but
...
limit on the \T2tt cross-section is shown in
