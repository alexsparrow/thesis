...
following goals.
\begin{itemize}
\item To inndicate the sensitivity of the analysis to the proposed model or set
...
$\theta$~\cite{statistical_methods}. Considered as a function of the parameter
...
measurement as consistent or less with the hypothesis $H$, than $X$.
...
supersymmetry search is covered in Appendix~\ref{sec:inter_1lepton}. In this
...
secondly, some discussion of validation work performed to demonstrate that the
...
To validate the model, a number of cross-checks were performed. Firstly, the
\ac{PL} and \CLs methods were compared and found to agree. In general, the \CLs
method is expected to be more conservative by convering the full range of the
...
(\Mgluino or \Mstop and \Mlsp) have been varied in steps of
\unit{20}{\GeV}. Since the mother particle be at least as massive as the
daughter half of the parameter space is unphysical and hence excluded.
...
space. This value was chosen by taking representative values of the \MET scale
...
assuming \ac{QCD}-strength couplings.
...
the modellling of \ac{ISR}. In this case the region, $\Mgluino - \Mlsp <
\unit{140}{\GeV}$ was ignored.
...
\caption[Limit in the \Ttwott simplified model]{Limit in the \Twott simplified
